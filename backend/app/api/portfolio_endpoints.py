...
def read_portfolio(portfolio_id: uuid.UUID, db: Session = Depends(get_db)):
...
def update_portfolio(portfolio_id: uuid.UUID, portfolio: PortfolioUpdate, db: Session = Depends(get_db)):
    db_portfolio = portfolio_crud.get_portfolio(db, portfolio_id=portfolio_id)
    if db_portfolio is None:
        raise HTTPException(status_code=404, detail="Portfolio not found")
